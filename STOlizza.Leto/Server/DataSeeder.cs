...
namespace STOlizza.Leto.Server
{
    public static class DataSeeder
    {
        ...
        {
            var samplesmena = context.Shifts.FirstOrDefault();
            if (samplesmena != null) return;

            var DefaultSmenaList = new List<SmenaDTO>(){
                new SmenaDTO(){number = 1, slogan="ПОМНИ", title = "Патриотика", dates = "20-23 июня", color = "#701487", availableTill = "10 июня", isAvailable = false},
                new SmenaDTO(){number = 2, slogan="УЗНАВАЙ", title = "Медиа", dates = "26-29 июня", color = "#701487", availableTill = "16 июня", isAvailable = false},
                new SmenaDTO(){number = 3, slogan="ДОСТИГАЙ", title = "Спорт", dates = "1-4 июля", color = "#701487", availableTill = "21 июня", isAvailable = false},
        ...
        {
            var samplerecord = context.Answers.FirstOrDefault();
            if (samplerecord != null) return;

            context.Add(new QuestionnairyDTO { Id = 0, Smena = 0, QImage = Array.Empty<byte>()/*, ImagePath = "test"*/, FirstName = "test", LastName = "test", FatherName = "test", BirthDate = DateTime.Today.AddYears(-18), Sex = "test", WorkingPlace = "test", Post = "test", PhoneNumber = "test", VkLink = "test", TelegramUsername = "test", ClothesSize = "test", Passport = "test", Allergies = "test", Illneses = "test", KnowledgeSource = "test", DesiredSkills = "test", ExpirienceIntentions = "test", VideoPath = "test" });
